...
var mmi = (function () {
    ...
    mmi.prototype.loadMaps = function (id,params) {
        if (params.key) { 
            var center=[28.61, 77.23],mhy=false,zm=15,mizm=4,mxzm=18;if(params.center) center=params.center; if(params.zoom) zm=params.zoom;if(params.minZoom) mizm=params.minZoom;if(params.maxZoom) mxzm=params.maxZoom;
			map_sdk = L.map(id, { zoomControl:false }).setView([28.61, 77.23], 13);
    ...
    mmi.prototype.geo = function (mpp,load,center,zoom,bound,html)
    {
        ...
        var clsid=document.getElementById('geo_location');console.log(clsid.className);
	...
	mmi.prototype.polyline = function (pts,options) {
		...
		if(pts)
		{
			...
			if(decorator){ var icon=decorator.icon;
...
// Installation constructor that binds MMI to window
mmi.install = function() {
	if (!window.plugins) {
	  window.plugins = {};
	}
	window.plugins.mmi = new mmi();
	return window.plugins.mmi;
  };
  
  // Bind all plugin to window object
  cordova.addConstructor(mmi.install);
